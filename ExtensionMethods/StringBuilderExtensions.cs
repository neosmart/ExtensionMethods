...
namespace NeoSmart.ExtensionMethods
{
    public static class StringBuilderExtensions
    {
        ...
        public static StringBuilder TrimEnd(this StringBuilder sb, params char[] values)
        {
            Array.Sort(values);
            for (int i = sb.Length - 1; i >= 0; --i)
            {
                // Abort when there's nothing left to trim
                if (Array.BinarySearch(values, sb[i]) < 0)
                {
